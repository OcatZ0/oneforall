...
class OpenSearchService
{
    ...
    public function getSecurityEventsMetrics($agentId, $timeRange = 'now-24h')
    {
        ...
        try {
            ...
            $r = $this->http()->withBasicAuth($this->_opensearchUser, $this->_opensearchPassword)->post("{$this->_opensearchHost}/wazuh-alerts-*/_search", $base);
            if ($r->successful()) $metrics['total'] = $r->json('hits.total.value') ?? 0;

            $q = $base; $q['query']['bool']['filter'][] = ['range' => ['rule.level' => ['gte' => 12]]];
            $r = $this->http()->withBasicAuth($this->_opensearchUser, $this->_opensearchPassword)->post("{$this->_opensearchHost}/wazuh-alerts-*/_search", $q);
    ...
    public function getAlertGroupsEvolution($agentId, $timeRange = '24h')
    {
        ...
        try {
            ...
            $response = $this->http()->withBasicAuth($this->_opensearchUser, $this->_opensearchPassword)->post("{$this->_opensearchHost}/wazuh-alerts-*/_search", $query);
    ...
    public function getPolicyAlertsOverTime($agentId, $timeRange = '24h')
    {
        $config = $this->getTimeRangeConfig($timeRange);
        $from   = match($timeRange) { 'today' => 'now/d', 'week' => 'now/w', default => "now-{$config['duration']}" };
        try {
            $query = [
                'size' => 0,
                'aggs' => ['alerts_over_time' => ['date_histogram' => ['field' => 'timestamp', 'fixed_interval' => $config['interval'], 'min_doc_count' => 0]]],
                'query' => ['bool' => [
                    'must'   => [['term' => ['agent.id' => $agentId]], ['match' => ['rule.groups' => 'rootcheck']]],
            ...
            if ($response->successful()) {
                ...
                return [
                    'labels' => array_map(fn($b) => Carbon::createFromTimestampMs($b['key'], 'UTC')->toIso8601String(), $buckets),
                    'data'   => array_map(fn($b) => $b['doc_count'], $buckets),
                ];
            }
        } catch (\Exception $e) {
            \Log::warning('Failed to fetch policy alerts over time: ' . $e->getMessage());
        }
        return ['labels' => [], 'data' => []];
    }

    public function getPolicyRuleDistribution($agentId, $timeRange = '24h', $limit = 5)
    {
        $config = $this->getTimeRangeConfig($timeRange);
        $from   = match($timeRange) { 'today' => 'now/d', 'week' => 'now/w', default => "now-{$config['duration']}" };
        try {
            $query = [
                'size' => 0,
                'aggs' => ['rule_distribution' => ['terms' => ['field' => 'rule.description', 'size' => $limit]]],
                'query' => ['bool' => [
                    'must'   => [['term' => ['agent.id' => $agentId]], ['match' => ['rule.groups' => 'rootcheck']]],
                    'filter' => [['range' => ['timestamp' => ['gte' => $from, 'lte' => 'now']]]],
                ]],
            ...
            if ($response->successful()) {
                ...
                return [
                    'labels' => array_map(fn($b) => substr($b['key'], 0, 50), $buckets),
                    'data'   => array_map(fn($b) => $b['doc_count'], $buckets),
                ];
            }
        } catch (\Exception $e) {
            \Log::warning('Failed to fetch policy rule distribution: ' . $e->getMessage());
        }
        return ['labels' => [], 'data' => []];
    }

    public function getPolicyControlTypeEvolution($agentId, $timeRange = '24h')
    {
        $config = $this->getTimeRangeConfig($timeRange);
        $from   = match($timeRange) { 'today' => 'now/d', 'week' => 'now/w', default => "now-{$config['duration']}" };
        try {
            $query = [
                'size' => 0,
                'aggs' => ['groups_evolution' => [
                    'date_histogram' => ['field' => 'timestamp', 'fixed_interval' => $config['interval'], 'min_doc_count' => 1],
                    'aggs' => ['groups' => ['terms' => ['field' => 'rule.groups', 'size' => 5]]],
                ]],
                'query' => ['bool' => [
                    'must'   => [['term' => ['agent.id' => $agentId]], ['match' => ['rule.groups' => 'rootcheck']]],
                    'filter' => [['range' => ['timestamp' => ['gte' => $from, 'lte' => 'now']]]],
